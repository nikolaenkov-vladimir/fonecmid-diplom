...
Процедура ВКМ_ВыполнитьАвтозаполнение(Договор, ДатаРеализации) Экспорт
	...
	Если ЗначениеЗаполнено(КонстантаРаботы) и 
		...
		//Начало изменений: Задача: ONEC-MID-DIPLOM. Автор: Николаенков В.А. Дата: 01.10.2024	  
		//Было:
		
		////Начало вставки: Задача: ONEC-MID-DIPLOM. Автор: Николаенков В.А. Дата: 09.09.2024		
		//Если Дата >= Договор.ВКМ_ДатаНачалаДействияДоговора и 
		//	Дата <= Договор.ВКМ_ДатаОкончанияДействияДоговора
		//	тогда    
		//	//Конец вставки	
		//	
		...
		ДанныеДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(Договор, "ВКМ_СуммаАбонентскойПлаты, ВКМ_ДатаНачалаДействияДоговора, ВКМ_ДатаОкончанияДействияДоговора");
		
		Если Дата >= ДанныеДоговора.ВКМ_ДатаНачалаДействияДоговора и Дата <= ДанныеДоговора.ВКМ_ДатаОкончанияДействияДоговора тогда    
			Если ДанныеДоговора.ВКМ_СуммаАбонентскойПлаты > 0 тогда
				
				СтрокаУслугиОбслуживание = Услуги.Добавить();
				СтрокаУслугиОбслуживание.Номенклатура = КонстантаАбонентскаяПлата; 
				СтрокаУслугиОбслуживание.Количество = 1;
				СтрокаУслугиОбслуживание.Цена = ДанныеДоговора.ВКМ_СуммаАбонентскойПлаты;
				СтрокаУслугиОбслуживание.Сумма = ДанныеДоговора.ВКМ_СуммаАбонентскойПлаты;	
		//Конец вставки
		//Конец изменений
